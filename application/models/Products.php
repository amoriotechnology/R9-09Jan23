...
public function get_products() {
 ...
 public function sales_product_all() {
         $sql='SELECT product_id,sum(quantity) as value FROM `invoice_details` GROUP by product_id';
        $query = $this->db->query($sql);
     //   echo $this->db->last_query();
        ...
        public function expense_product_all() {
         $sql='SELECT product_id,sum(quantity) as value FROM `product_purchase_details` GROUP by product_id';
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
